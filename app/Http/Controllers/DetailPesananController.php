    ...
    public function showDetailPesanan(Request $request)
{
    ...
    if (empty($request->input('search.value'))) {
        ...
        $detailPesanan_data = DetailPesanan::where('id', 'LIKE', "%{$search_text}%")
        ...
        $totalFilteredRecord = DetailPesanan::where('id', 'LIKE', "%{$search_text}%")
            ->orWhere('idPesanan', 'LIKE', "%{$search_text}%")
            ->orWhere('idMakanan', 'LIKE', "%{$search_text}%")
            ->orWhere('hargasatuan', 'LIKE', "%{$search_text}%")
            ->orWhere('jumlah', 'LIKE', "%{$search_text}%")
    ...
    if (!empty($detailPesanan_data)) {
        foreach ($detailPesanan_data as $detailPesanan_val) {
            $url = route('user.detail-pesanan.user.edit', ['id' => $detailPesanan_val->id]);
            $urlHapus = route('user.detail-pesanan.user.delete', $detailPesanan_val->id);

            $detailPesananNestedData = [
                'id' => $detailPesanan_val->id, // Change 'idDetail' to 'id'
    ...
    public function ubahDetailPesanan($id, Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->route('user.detail-pesanan.user.edit', ['id' => $detailPesanan->idDetail])
                ->with('status', 'Data detail pesanan telah tersimpan di database');
        }

        return view('page.admin.detail_pesanan.user.ubahDetailPesanan', compact('detailPesanan'));
    }
    ...
    public function tambahDetailPesanan(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            return redirect()->route('detail-pesanan.user.add')->with('status', 'Data telah tersimpan di database');
        }
